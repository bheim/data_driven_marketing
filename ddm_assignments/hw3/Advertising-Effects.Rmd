...
```{r, eval=FALSE}
merged_data <- merge(move_data, advertising_data, by = c("brand_name", "dma_code", "week_end"))
```
## Reshape the data
...
```{r}
wide_data <- dcast(merged_data, store_code_uc + week_end ~ brand_name, value.var = c("quantity", "price", "promotion", "adstock"))
clean_merged_data <- wide_data[complete.cases(wide_data), ]
...
```{r, eval = FALSE}
move[, week_end := as.Date(week_end, format = "%Y-%m-%d")]
move[, month_index := 12 * (as.integer(format(week_end, "%Y")) - 2011) + as.integer(format(week_end, "%m"))]
head(move[, .(week_end, month_index)]) #sanity check
```
