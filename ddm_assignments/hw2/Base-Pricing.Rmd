...
The above estimation results highlight the importance of incorporating store fixed effects, time trends, competitor prices, and promotions to accurately model the demand for Tide. First, by including store fixed effects, the model output showed in significant improvement in performance, as reflected in the notable increase in R-squared values. Without controlling for these fixed effects, the estimated own and cross-price elasticities would be biased, as demonstrated by the shift in the cross-price elasticity between Tide and Gain once store-level differences were accounted for, and could suggest that Tide and Gain are substitutes in certain markets.

Also, using time trends and fixed effects, such as month and year controls, only marginally improved the model's fit, capturing the effects of seasonality, for example. While these time controls contributed to a more comprehensive model, their impact on the core elasticity estimates was relatively minor -- suggesting that store-specific factors are much more critical to understanding consumer behavior.

Finally, competitor prices and promotions also played a crucial role in accurately estimating demand. Tide’s own-price elasticity remained significant across all models, showing a high degree of price sensitivity. However, the magnitude of this sensitivity was reduced when promotions were included, indicating that promotions mitigate the negative impact of price increases. Tide’s promotions, in particular, had a substantial positive effect on sales, driving increases in demand ranging from 34% to 42%. On the other hand, competitor promotions—particularly from Gain—had a negative effect on Tide’s sales.
