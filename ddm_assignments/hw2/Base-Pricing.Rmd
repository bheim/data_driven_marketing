...
```{r}
final_data_with_store[,, by = week_end]
```
...
fit_base <- feols(log(1 + quantity_503626) ~ log(price_503626), data = final_data_with_store)

# Add store fixed effects
fit_store_FE <- feols(log(1 + quantity_503626) ~ log(price_503626) | store_code_uc, data = final_data_with_store)

# Add a linear time trend
fit_trend <- feols(log(1 + quantity_503626) ~ log(price_503626) + month_trend | store_code_uc, data = final_data_with_store)

# Add fixed effects for each year/month
fit_month_FE <- feols(log(1 + quantity_503626) ~ log(price_503626) | store_code_uc + year + month, data = final_data_with_store)
```
