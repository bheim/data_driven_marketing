...
fit_month_FE_competitor <- feols(
  log(1 + quantity_Tide) ~ log(price_Tide) + log(price_Gain) + 
    log(price_ArmHammer) + log(price_Purex) | store_code_uc + year + month, data = final_data_with_store)
...
fit_trend_competitor_promo <- feols(
  log(1 + quantity_Tide) ~ log(price_Tide) + log(price_Gain) + 
    log(price_ArmHammer) + log(price_Purex) + month + promotion_Tide | store_code_uc,
  data = final_data_with_store
)

fit_month_FE_competitor_promo <- feols(
  log(1 + quantity_Tide) ~ log(price_Tide) + log(price_Gain) + 
    log(price_ArmHammer) + log(price_Purex) + promotion_Tide | store_code_uc + year + month,
  data = final_data_with_store
)
