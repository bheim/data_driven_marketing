...
data <- data.frame(
  ...
  product_name = c('TIDE - H-D LIQ', 'PUREX - H-D LIQ', 'GAIN - H-D LIQ', 'ARM & HAMMER - H-D LIQ', 'ALL - H-D LIQ'),
  category = c('DETERGENTS - HEAVY DUTY - LIQUID', 'DETERGENTS - HEAVY DUTY - LIQUID', 'DETERGENTS - HEAVY DUTY - LIQUID', 'DETERGENTS - HEAVY DUTY - LIQUID', 'DETERGENTS - HEAVY DUTY - LIQUID')#,
...
library(data.table) 
setnames(move, old = "promo_dummy", new = "promotion")
setnames(move, old = "units", new = "quantity")
setnames(move, old = "brand_code_uc", new = "product_id")
...
final_data[, outliers := isOutlier(price, threshold_bottom, threshold_top), by = .(store_code_uc, brand_name)]

outlier_flags = final_data[, isOutlier(price, threshold_bottom, threshold_top), by = .(store_code_uc, brand_name)]
total_outliers = sum(outlier_flags$V1)
print(paste("Total number of outliers:", total_outliers))
final_data = final_data[!outlier_flags$V1]
...
```{r}
<<<<<<< HEAD
load("~/Desktop/data_driven_marketing/ddm_assignments/Data/Stores.RData")
ls() 
=======
load("data/Stores.RData")
ls()
>>>>>>> 3703d788efa3bdf85a82756b3e28ca60af68f87b
print(stores)   
```
```{r}
...
```{r}
final_data_with_store[, .N]
```
Number of stores:
```{r}
final_data_with_store[, uniqueN(store_code_uc)]
```
...
Understanding price variation is important for 3 reasons: 1. extrapolation limits, 2. prices vary by region, 3. the goal of determining elasticity. For the first reason, it is essential we understand how much prices vary as when we create our model, we will not want to extrapolate the model too far beyond where we have data. As far as reason 2, there is likely significant variation across regions and the degree of variation is important to understand when we consider what we should account for in our model, and 3. we have a goal of determining price elasticity and we need to ensure that we have significant variation to project elasticity - if prices rarely vary and only vary a small amount, we will have limited predictive power (due to reason 1).
...
```{r}
tide_average = final_data_with_store[, mean(price_Tide)]
norm_tide = final_data_with_store[, price_Tide/tide_average]

x_limits <- quantile(df$norm_tide, probs = c(0.05, 0.95), na.rm = TRUE)
df = data.frame(norm_tide)
ggplot(df, aes(x = norm_tide)) + 
  geom_histogram(binwidth = 0.1, fill = "lightblue", color = "black") +
  
  # Customize x-axis tick marks
  scale_x_continuous(breaks = seq(min(df$norm_tide), max(df$norm_tide), by = 0.2), 
...
gain_average = final_data_with_store[, mean(price_Gain)]
norm_gain = final_data_with_store[, price_Gain / gain_average]

df = data.frame(norm_gain)
ggplot(df, aes(x = norm_gain)) + 
  geom_histogram(binwidth = 0.1, fill = "lightblue", color = "black") +
  
  # Customize x-axis tick marks for normalized prices
 scale_x_continuous(breaks = seq(min(df$norm_gain), max(df$norm_gain), by = 0.2), 
                     labels = number_format(accuracy = 0.1)) +
