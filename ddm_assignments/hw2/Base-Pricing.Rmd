...
```{r}
avg_price_Tide = mean(final_data_with_store$price_Tide)
avg_price_Gain = mean(final_data_with_store$price_Gain)
for (i in 1:nrow(profit_DT)) {
   ...
   coef(fit_Tide)[1] + coef(fit_Tide)[2] * log(new_price_Tide)
   predicted_quantity_Tide <- exp(coef(fit_Tide)[1] * log(new_price_Tide) + coef(fit_Tide)[2] * log(new_price_Gain)) - 1
   predicted_quantity_Gain <- exp(coef(fit_Gain)[1] * log(new_price_Gain) + coef(fit_Gain)[2] * log(new_price_Tide)) - 1
