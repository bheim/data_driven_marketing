...
# First, add promotion dummy for Tide only
fit_promo_Tide <- feols(
  log(1 + quantity_Tide) ~ log(price_Tide) + promotion_Tide | store_code_uc + year + month,
  data = final_data_with_store
)

# Second, add promotion dummies for all brands (Tide and Gain)
fit_promo_all <- feols(
  log(1 + quantity_Tide) ~ log(price_Tide) + log(price_Gain) + promotion_Tide + promotion_Gain | store_code_uc + year + month,
  data = final_data_with_store
)

etable(fit_base_competitor, fit_promo_Tide, fit_promo_all,
       tex = TRUE,
       fitstat = c("n", "r2"),
       signif.code = NA,
       cluster = c("store_code_uc", "month_trend"))
...
The estimation results give a lot of information on demand for Tide, especially after controlling for competitor prices, promotions, store-level fixed effects, and time trends. Initially, without controls, the own price elasticity for Tide was very large, which shows extreme price sensitivity. As more controls were added, including store-specific fixed effects and competitor prices, the own price elasticity stabilized at around -4.18. This shows that controlling for these factors reduced potential bias in the estimates and provided a more realistic assessment of Tide's price sensitivity. Of note, the magnitude of the elasticity still reflects a high level of responsiveness to price changes, underscoring the importance of pricing strategies for this product.

The inclusion of competitor prices, particularly Gain's price, significantly improved the model's accuracy. Initially, without controls, the cross-price elasticity for Gain was negative, which contradicts the expected substitution relationship between the two products. However, after controlling for store fixed effects and other factors, the cross-price elasticity became positive, confirming that Tide and Gain are substitutes. This finding highlights the importance of accounting for competitor pricing in demand models to accurately capture the dynamics of consumer behavior in the marketplace.

Promotions also played a key role in driving Tide's demand. The model shows that Tide's promotions lead to a significant increase in sales, with promotions boosting demand by approximately 35%. This suggests that promotions are a highly effective tool in influencing purchasing decisions. Failing to control for promotions could lead to an overstatement of price elasticity, as promotional activities and price changes are often correlated. 
